...
def get_image_name_prefix():
    ...
    try:
        ...
        errors_and_warnings.append(f"Error: Settings file not found at {SETTINGS_TXT}")
        return "image"
    except Exception as e:
        errors_and_warnings.append(f"Error reading settings.txt: {e}")
        return "image"
...
def read_shop_names():
    ...
    try:
        ...
        errors_and_warnings.append(f"Error: Shops TSV file not found at {SHOPS_TSV}")
        return []
    except Exception as e:
        errors_and_warnings.append(f"Error reading shops.tsv: {e}")
        return []
...
def clean_filename(name, index, prefix):
    try:
        ...
        # Add a prefix, number, followed by shop name
        ...
        errors_and_warnings.append(f"Error cleaning filename for '{name}': {e}")
        return f"{prefix}-{index + 1}-unnamed"
...
def convert_to_webp(input_path, output_path):
    ...
    try:
        ...
        errors_and_warnings.append(f"Conversion error {os.path.basename(input_path)}: {str(e)}")
...
def ensure_directory_exists(directory):
    ...
    try:
        ...
        errors_and_warnings.append(f"Error creating directory {directory}: {e}")
        raise  # Re-raise to be caught by the caller
...
def process_images():
    try:
        ...
        try:
            for filename in os.listdir(OUTPUT_DIR):
                ...
                try:
                    ...
                    errors_and_warnings.append(f'Failed to delete {file_path}. Reason: {e}')
        except Exception as e:
            errors_and_warnings.append(f"Error cleaning output directory: {e}")
        
        # Check if input directory exists
        if not os.path.exists(INPUT_DIR):
            errors_and_warnings.append(f"Error: Input directory not found at {INPUT_DIR}")
            return
        ...
        if not input_files:
            errors_and_warnings.append("No image files found in input directory")
            return
        ...
        if not massage_shops:
            errors_and_warnings.append("Warning: No shop names found in TSV. Using default naming.")
            ...
            try:
                ...
                if number_match:
                    ...
                    if file_number < len(massage_shops):
                        ...
                        new_base_name = f"{prefix}-{file_number + 1}-unnamed"
                else:
                    # For files that don't match the numbered pattern, keep original name with prefix
                    base_name = os.path.splitext(original_filename)[0]
                    # Maintain consistent format with number before name for special files
                    new_base_name = f"{prefix}-special-{base_name}"
                ...
                errors_and_warnings.append(f"Error processing file {original_filename}: {e}")
                failed_conversions += 1
        
        print(f"\nProcessing complete: {successful_conversions} successful, {failed_conversions} failed")
...
if __name__ == "__main__":
    try:
        ...
        errors_and_warnings.append(f"Critical error: {str(e)}")
        errors_and_warnings.append(traceback.format_exc())
