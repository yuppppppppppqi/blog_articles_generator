...
def get_image_mapping():
    ...
    for filename in os.listdir(IMAGES_DIR):
        if filename.endswith('.webp'):
            ...
            match = re.search(r'foot-massage-sukhumvit-(\d+)-', filename)
            if match:
...
TEMPLATE_DIR = './contents/prompt_script_template'
BASE_DIR = './contents'
...
def get_image_filename(target_chapter):
    ...
    return IMAGE_FILENAMES.get(target_chapter, "")

def get_image_alt(target_chapter):
    """Generate the image alt text based on target chapter."""
    return IMAGE_ALTS.get(target_chapter, "")

def read_summary_content():
    """Read the content of summary file."""
    try:
        with open('summary_of_features_and_highlights_of_all_shops.txt', 'r', encoding='utf-8') as f:
            return f.read()
    except FileNotFoundError:
        print("Warning: summary_of_features_and_highlights_of_all_shops.txt not found")
        return ""
                ...
                if i == target_chapter:
                    if row:  # Check if row is not empty
                        return {
                            ...
                            'iframe_code': row[5] if len(row) > 5 else ""  # Add iframe_code from column 6 (index 5)
                        }
    except FileNotFoundError:
