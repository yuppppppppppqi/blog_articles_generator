...
def generate_summary_html(project_name, base_temp_path_project, section_num, summary_txt_path, output_path):
    ...
    rating_text = lines[0] if len(lines) > 0 else "Нет информации об оценке"
    price_text = lines[1] if len(lines) > 1 else "Нет информации о цене"
    recommended_text = lines[2] if len(lines) > 2 else "Нет рекомендаций" # summary.txtの3行目は「おすすめ」で始まる想定
    summary_body_text = lines[3] if len(lines) > 3 else "Нет описания" # summary.txtの4行目は「まとめ」で始まる想定
    ...
    # --- 英語のショップ名を取得する処理 ---
    shop_name_en = "Unknown Shop Name" # デフォルト
    # 3_generate_html.py の実行場所からの相対パスで英語版プロジェクトのパスを構築
    # このスクリプトは translation/3_generate_html.py なので、親ディレクトリがワークスペースルート
    script_dir = os.path.dirname(os.path.abspath(__file__)) # translation/ru
    workspace_root = os.path.dirname(os.path.dirname(script_dir)) # blog_articles_generator
            ...
            with open(english_shops_tsv_path, 'r', encoding='utf-8') as f_tsv:
                ...
                for i, row in enumerate(tsv_reader, start=1):
                    ...
                    if i == section_num:
                        ...
                        print(f"  [DEBUG] section_num={section_num}, shop_name_en={shop_name_en}")
                        break
                if shop_name_en == "Unknown Shop Name" and section_num <= i:
...
<ul class='wp-block-list' style="padding-left: 15px; margin-left: 0;">
   <!-- wp:list-item -->
   <li style="margin-bottom: 10px; margin-left: 0;"><strong>Общая оценка:</strong> {rating_ja}</li>
   <!-- /wp:list-item -->
   <!-- wp:list-item -->
   <li style="margin-bottom: 10px; margin-left: 0;"><strong>Цена:</strong> {price_ja}</li>
   <!-- /wp:list-item -->
   <!-- wp:list-item -->
   <li style="margin-bottom: 10px; margin-left: 0;"><strong>Рекомендуется для:</strong> {recommended_ja}</li>
   <!-- /wp:list-item -->
</ul>
<!-- /wp:list -->
<!-- wp:paragraph -->
...
def generate_reviews_html(reviews_txt_path, output_path):
    ...
    html += '<h3 class="wp-block-heading"><strong>Отзывы</strong></h3>\n'
    html += '<!-- /wp:heading -->\n\n'
    html += '<!-- wp:list -->\n'
...
def generate_conclusion_with_h2(conclusion_path, output_path):
    ...
    if not content:
        return ""
    h2_tag = '<h2 style="margin-bottom: 20px; margin-top: 50px;"><strong>Итог</strong></h2>'
    new_content = h2_tag + '\n' + content
...
def main():
    ...
    for project_name_translation in sorted(project_dirs_translation, key=lambda x: int(x.split('_')[0])):
        try:
            ...
            output_file_project_translation = os.path.join(output_dir_project_translation, f"{LANG_CODE}.html")
            os.makedirs(output_dir_project_translation, exist_ok=True)

            html_parts = []

            # intro
            intro_path_translation = os.path.join(base_temp_path_project_translation, "sections", "intro", "intro.html")
            if not os.path.exists(intro_path_translation):
                fallback_intro = os.path.join("translation", "ru", "intro_results", f"intro_ru_{project_name_translation}.txt")
                if os.path.exists(fallback_intro):
                    intro_path_translation = fallback_intro
            ...
            for sec_num in section_numbers:
                sec_dir_translation = os.path.join(sections_dir_translation, str(sec_num), LANG_CODE) # ロシア語版のセクションパス
                ...
                if os.path.exists(summary_txt_translation):
                    ...
                    # summary_resultsから探す
                    summary_result_path = os.path.join("translation", "ru", "summary_results", f"summary_ru_{project_name_translation}_{sec_num}.txt")
                    if os.path.exists(summary_result_path):
                        summary_html_content = generate_summary_html(project_name_translation, base_temp_path_project_translation, sec_num, summary_result_path, summary_html_output_path)
                        if summary_html_content: html_parts.append(summary_html_content)
                    else:
                        print(f"  Warning: summary.txt not found for section {sec_num} in {project_name_translation} and no Gemini summary result found.")
                ...
                if os.path.exists(reviews_txt_translation):
                    ...
                    # reviews_resultsから探す
                    reviews_result_path = os.path.join("translation", "ru", "reviews_results", f"reviews_ru_{project_name_translation}_{sec_num}.txt")
                    if os.path.exists(reviews_result_path):
                        reviews_html_content = generate_reviews_html(reviews_result_path, reviews_html_output_path)
                        if reviews_html_content: html_parts.append(reviews_html_content)
                    else:
                        print(f"  Warning: reviews.txt not found for section {sec_num} in {project_name_translation} and no Gemini reviews result found.")

            # conclusion
            conclusion_dir_translation = os.path.join(base_temp_path_project_translation, "sections", "conclusion")
            conclusion_html_translation = os.path.join(conclusion_dir_translation, "conclusion.html")
            if not os.path.exists(conclusion_html_translation):
                fallback_conclusion = os.path.join("translation", "ru", "conclusion_results", f"conclusion_ru_{project_name_translation}.txt")
                if os.path.exists(fallback_conclusion):
                    conclusion_html_translation = fallback_conclusion
            ...
            # --- まとめ（conclusion）を必ず最後に追加 ---
            # 既にconclusionがhtml_partsに含まれていない場合のみ追加
            conclusion_ru_sukhumvit = os.path.join("translation", "ru", "conclusion_results", "conclusion_ru_Sukkhumvit.txt")
            if os.path.exists(conclusion_ru_sukhumvit):
                with open(conclusion_ru_sukhumvit, 'r', encoding='utf-8') as f:
                    conclusion_content = f.read()
                # 既にfinal_htmlにconclusion_contentが含まれていない場合のみ追加
                if conclusion_content.strip() and conclusion_content.strip() not in final_html:
