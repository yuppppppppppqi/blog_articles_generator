...
def process_project(project_name):
    ...
    try:
        with open(focus_keywords_file_path, 'r', encoding='utf-8-sig') as csvfile: # Use utf-8-sig for BOM handling
            ...
            found_ru = False
            for row in reader:
                if row.get('language','').strip().lower() == LANG_CODE:
                    found_ru = True
                    ...
                    break # Found Russian row, no need to continue
            if not found_ru:
                print(f"Warning: No '{LANG_CODE}' language row found in {focus_keywords_file_path} for {project_name}.")
    ...
    if not primary_topic_keyword:
        print(f"Warning: Primary Focus Keyword (topic) for '{LANG_CODE}' is empty/missing in {focus_keywords_file_path} for {project_name}. Title/SEO affected.")
        primary_topic_keyword = "[Топик]" # Placeholder if empty
    if not secondary_location_keyword:
        print(f"Warning: Secondary Focus Keyword (location) for '{LANG_CODE}' is empty/missing in {focus_keywords_file_path} for {project_name}. Title affected.")
        secondary_location_keyword = "[Место]" # Placeholder if empty

    if not seo_keywords_list_for_constraint:
        seo_keywords_constraint_text = "SEO ключевые слова должны быть включены в создание." 
    else:
        seo_keywords_constraint_text = "SEO ключевые слова должны включать " + "и".join([f'"{kw}"' for kw in seo_keywords_list_for_constraint]) + "."
    ...
    article_title = f'”【Живущие выбирают】{secondary_location_keyword} {primary_topic_keyword} рекомендуемые {num_shops} выбор [2025 год]”'
    ...
    # 追加: intro_promptsにもプロジェクト名入りでコピー
    intro_prompts_dir = os.path.join(os.path.dirname(__file__), "intro_prompts")
    os.makedirs(intro_prompts_dir, exist_ok=True)
    intro_prompt_projectfile = os.path.join(intro_prompts_dir, f"body_intro_prompt_ru_{project_name}.txt")
    with open(intro_prompt_projectfile, 'w', encoding='utf-8') as f: f.write(intro_prompt_content)
    print(f"Generated: {intro_prompt_projectfile}")
    ...
    # 追加: conclusion_promptsにもプロジェクト名入りでコピー
    conclusion_prompts_dir = os.path.join(os.path.dirname(__file__), "conclusion_prompts")
    os.makedirs(conclusion_prompts_dir, exist_ok=True)
    conclusion_prompt_projectfile = os.path.join(conclusion_prompts_dir, f"body_conclusion_prompt_ru_{project_name}.txt")
    with open(conclusion_prompt_projectfile, 'w', encoding='utf-8') as f: f.write(conclusion_prompt_content)
    print(f"Generated: {conclusion_prompt_projectfile}")
    ...
    for i, shop_tsv_data_line in enumerate(shop_data_lines):
        ...
        output_section_dir = os.path.join(output_sections_base_dir, str(section_num), LANG_CODE)
