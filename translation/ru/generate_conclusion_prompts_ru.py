...
CONCLUSION_PROMPT_DIR = os.path.join(os.path.dirname(__file__), "conclusion_prompts")
os.makedirs(CONCLUSION_PROMPT_DIR, exist_ok=True)
...
CONCLUSION_TEMPLATE = '''必ずロシア語で出力してください。日本語で出力しないでください。

{ARTICLE_TITLE}というタイトルに相応しい魅力的なアウトロ（まとめ）を書いてください。
...
Output:
translation/ru/temp/{PROJECT_NAME}/sections/conclusion/conclusion.html
...
for fname in os.listdir(TITLE_DIR):
    if fname.startswith("title_ru_") and fname.endswith(".txt"):
        ...
        prompt = CONCLUSION_TEMPLATE.replace("{ARTICLE_TITLE}", title) \
            .replace("{SEO_KEYWORDS_CONSTRAINT}", "") \
            .replace("{ALL_SHOPS_FEATURES_AND_HIGHLIGHTS}", all_shops_features)
        if dir_name:
            prompt = prompt.replace("{PROJECT_NAME}", dir_name)
        out_path = os.path.join(CONCLUSION_PROMPT_DIR, f"body_conclusion_prompt_ru_{area}.txt")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(prompt)
        print(f"{out_path} にロシア語コンクルージョン生成用プロンプトを出力しました。") 