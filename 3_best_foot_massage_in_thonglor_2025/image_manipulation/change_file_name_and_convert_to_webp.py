...
KEYWORDS_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data_input", "focus_keywords.csv")
...
def get_english_keywords():
    ...
    try:
        with open(KEYWORDS_CSV, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if row['language'].strip() == 'en':
                    primary = row['Primary Focus Keyword'].strip().lower()
                    secondary = row['Secondary Focus Keyword'].strip().lower()
                    # Convert to lowercase and replace spaces with hyphens
                    prefix = f"{primary}-{secondary}".replace(' ', '-')
                    return prefix
    except Exception as e:
        print(f"Error reading focus_keywords.csv: {e}")
        return "foot-massage-thong-lof"  # fallback prefix with correct location
...
def process_images():
    ...
    # Get prefix from English focus keywords
